...
classes_to_test={'averaged_perceptron_multilabel_classifier'} #{'averaged_perceptron_multilabel_classifier', 'perceptron_classifier'}
...
class Perceptron_Multilabel_Classifier(Classifier): 
	...
	def train(self,X,Y, warm_start=True):
		...
		for i in idx:
			...
			def make_prediction():
				def get_sum():
					return sp.csr_matrix.dot(row, self.percep_mat.T)
				sums = get_sum()
				...
				return best_index,best_label,correct_index
			best_index,best_label,correct_index=make_prediction()
			def update():
				if best_label != Y[i]:
					...
					self.percep_mat[best_index,row.indices] -=  row.data
					self.percep_mat[correct_index,row.indices] +=  row.data
			update()
...
#				self.percep_mat[correct_index,:] +=  X[i,:]
				self.percep_mat[best_index,X[i,:].indices] -=  X[i,:].data
				self.percep_mat[correct_index,X[i,:].indices] +=  X[i,:].data
